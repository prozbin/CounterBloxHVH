...
aimbot:Element('Slider', 'rayamount', {min = 1, max = 3, default = 1})     
				...
				for _,Player in pairs(Players:GetPlayers()) do      
					...
					if Player.Character and Player.Character:FindFirstChild("Humanoid") and Player.Character:FindFirstChild("Humanoid").Health > 0 and Player.Team ~= "TTT" and Player ~= LocalPlayer then      
						...
						if TBLFIND(values.rage.aimbot.resolver.Jumbobox, 'arms') then
							Player.Character.RightUpperArm:FindFirstChildWhichIsA('Motor6D').C0 = CFrame.new(1.5 * (values.rage.aimbot['resolver offset distance'].Slider / 10), 0.549999952, -0.2)
							Player.Character.LeftUpperArm:FindFirstChildWhichIsA('Motor6D').C0 = CFrame.new(-(1.5 * (values.rage.aimbot['resolver offset distance'].Slider / 10)), 0.549999952, -0.2)
						end
