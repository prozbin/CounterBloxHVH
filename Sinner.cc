...
game.StarterGui:SetCore("SendNotification", {
        Title = "Spinner.cc";
        Text = "Thank you for Using Spinner.cc BETA!";
        Duration = 7;
})
...
local gui = library:New("Spinner.cc")      
...
aimbot:Element('Dropdown', 'wallbang methods', {options = {'force', 'true', 'none'}})      
...
aimbot:Element("Toggle", "forward track shoot helper")
aimbot:Element("Dropdown", "prediction mode", {options = {'sexpackage', '$$$', 'cframe', 'velocity', 'none'}})     
...
aimbot:Element('Slider', 'rayamount', {min = 1, max = 10})     
aimbot:Element("Toggle", "teammates")      
aimbot:Element("Toggle", "auto baim")
aimbot:Element("Toggle", "shoot before round start")
aimbot:Element("Toggle", "hit logs")
aimbot:Element('Slider', 'log time', {min = 1, max = 10, default = 2})  
aimbot:Element("Toggle", "HS") 
aimbot:Element('Slider', "HPS Speed", {min = 1, max = 66, default = 1}) 
...
antiaim:Element("Slider", "spin speed", {min = 0.5, max = 48, default = 4})      
antiaim:Element('ToggleKeybind', 'onshot anti-aim')
antiaim:Element('Dropdown', 'onshot mode', {options = {"none", "up", "down", "zero", "180", "random"}})
...
local buybot = rage:Sector("Buy Bot", "Right")
buybot:Element('Button', 'Buy USP', {}, function()
	-- pasted from google
	local gun = "USP"
	local givefunc
...
buybot:Element('Button', 'Buy P90', {}, function()
	-- pasted from google
	local gun = "P90"
	local givefunc
	for _, v in pairs(getgc()) do
...
buybot:Element('Button', 'Buy AWP', {}, function()
	-- pasted from google
	local gun = "AWP"
	local givefunc
	for _, v in pairs(getgc()) do
...
buybot:Element('Button', 'Buy AK47', {}, function()
	-- pasted from google
	local gun = "AK47"
	local givefunc
	for _, v in pairs(getgc()) do
...
buybot:Element('Button', 'Buy Galil', {}, function()
	-- pasted from google
	local gun = "Galil"
	local givefunc
	for _, v in pairs(getgc()) do
...
buybot:Element('Button', 'Buy DesertEagle', {}, function()
	-- pasted from Earth
...
buybot:Element('Button', 'Buy C4', {}, function()
	-- pasted from Earth
	local gun = "C4"
	local givefunc
	for _, v in pairs(getgc()) do
...
buybot:Element('Button', 'Buy Tec9', {}, function()
	-- pasted from Earth
	local gun = "Tec9"
	local givefunc
	for _, v in pairs(getgc()) do
...
exploits:Element('Toggle', 'backtrack')
exploits:Element('Slider', 'backtrack ms', {min = 1, max = 100, default = 10})
...
exploits:Element("Button", "walkspeed speed", {}, function(tbl)
	-- Speed
...
exploits:Element('Slider', 'speed speed', {min = 1, max = 100, default = 1}) 
...
effects:Element('Dropdown', 'forcefield type', {options = {'normal', 'bloxsenseripoff', 'drugs', 'disgusting'}})      
...
self:Element("ToggleTrans", "self chams", {default = {Color = COL3RGB(255,255,255), Transparency = 0}}, function(tbl)      
	if tbl.Toggle then      
		for _,obj in pairs(SelfObj) do      
			if obj.Parent ~= nil then      
				obj.Material = Enum.Material.ForceField      
...
local client = misc:Sector("extra", "Right")
...
movement:Element("Toggle", "cancel hop")
...
local speed = values.visuals.self["silent angle speed"].Slider/35
local last = tick()
...
mt.__namecall = function(self, ...)      
	...
	if method == "FireServer" and self.Name == "HitPart" then      
			...
			coroutine.wrap(function()
				if game.Players:GetPlayerFromCharacter(args[1].Parent) then RageTarget = args[1] end
					if Players:GetPlayerFromCharacter(args[1].Parent) or args[1] == RageTarget then
						if values.rage.aimbot['forward track shoot helper'].Toggle then
							if Players:GetPlayerFromCharacter(args[1].Parent) or args[1] == RageTarget then 
		...
		if (values.rage.aimbot["forward track shoot helper"].Toggle and RageTarget ~= nil) then      
			coroutine.wrap(function()      
				if Players:GetPlayerFromCharacter(args[1].Parent) or args[1] == RageTarget then      
					local hrp = RageTarget.Parent.HumanoidRootPart.Position      
					local oldHrp = RageTarget.Parent.HumanoidRootPart.OldPosition.Value      
