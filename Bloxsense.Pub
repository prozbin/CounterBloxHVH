...
makefolder("bloxsense_luas")      

local allluas = {}  

for _,lua in pairs(listfiles("bloxsense_luas")) do  
	local luaname = GSUB(lua, "bloxsense_luas\\", "")  
	INSERT(allluas, luaname)  
end  

RunService.RenderStepped:Wait()      

local gui = library:New("bloxsense.")      
...
luascripts:Element("Button", "load", {}, function()  
	loadstring(readfile("bloxsensluas\\"..values.luas["lua scripts"].lua.Scroll))()  
end)  
...
luascripts:Element("Button", "Old Gun Sounds", nil, function()
	loadstring(game:HttpGet("https://raw.githubusercontent.com/farthefarter/old/main/gun"))()
end) 
...
aimbot:Element("Jumbobox", "resolver", {options = {"pitch", "Bhop", "Brute Force", "roll","animation"}})    
aimbot:Element("Dropdown", "wallbangs", {options = {"normal", "force"}})  
...
aimbot:Element("Toggle", "super prediction")     
aimbot:Element("Toggle", "teammates") 
aimbot:Element("Toggle", "backtrack")       
aimbot:Element("Slider", "backtrack ms", {min = 0, max = 400, default = 1})          
...
aimbot:Element("Dropdown", "autowall type", {options = {"cw", "standard", "off"}}) 
...
local function AddRage(Tab)      
	Tab:Element("Jumbobox", "hitboxes", {options = {"head", "torso", "pelvis", "legs", "feet"}})      
	Tab:Element("Toggle", "prefer body")      
	Tab:Element("Slider", "minimum damage", {min = -1000, max = 100, default = 1})      
...
antiaim:Element("Dropdown", "pitch", {options = {"none", "up", "down", "dogshit", "180", "funny", "bad", "bad2", "bad3", "random", "God", "fucker", "cool", "cool2", "back"}})  
...
antiaim:Element("Dropdown", "body roll", {options = {"off", "on", "-4", "-15", "-30", "-60", "-180", "-360", "-374", "-262", "spin"}})
...
exploits:Element("Slider", "quick peek pos", {min = -500, max = 500, default = 200})  
	...
	if tbl.Toggle and tbl.Active and LocalPlayer.Character and Peek == false then
		if values.rage.exploits["loop peek"].Toggle == true and values.rage.exploits["quick peek"].Active == true then
			while values.rage.exploits["loop peek"].Toggle == true do
				Peek = true
				LocalPlayer.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame + Vector3.new(0,values.rage.exploits["quick peek pos"].Slider,0)
				wait(0.2)
				Peek = false
				LocalPlayer.Character.HumanoidRootPart.CFrame = LocalPlayer.Character.HumanoidRootPart.CFrame - Vector3.new(0,values.rage.exploits["quick peek pos"].Slider,0)
				wait(0.2)
...
self:Element("Slider", "viewmodel x", {min = -35, max = 35}, function(val)      
...
self:Element("Slider", "viewmodel y", {min = -35, max = 35}, function(val)      
	ViewmodelOffset = CF(values.visuals.self["viewmodel x"].Slider/7, values.visuals.self["viewmodel y"].Slider/7, values.visuals.self["viewmodel z"].Slider/7) * CFAngles(0, 0, values.visuals.self.roll.Slider/50)      
end)      
self:Element("Slider", "viewmodel z", {min = -35, max = 35}, function(val)      
	ViewmodelOffset = CF(values.visuals.self["viewmodel x"].Slider/7, values.visuals.self["viewmodel y"].Slider/7, values.visuals.self["viewmodel z"].Slider/7) * CFAngles(0, 0, values.visuals.self.roll.Slider/50)      
end)      
...
world:Element("ToggleColor", "impacts", {default = {Color = COL3RGB(168, 124, 222)}})      
world:Element("ToggleColor", "hit chams", {default = {Color = COL3RGB(0, 0, 255)}})      
world:Element("Dropdown", "hitsound", {options = {"none", "skeet", "neverlose", "rust", "bag", "baimware", "1nn", "Cod", "Bonk", "Welcome", "Semi", "osu", "Tf2", "Tf2 pan", "M55solix", "Slap","Minecraft", "jojo", "vibe", "supersmash", "epic", "retro", "funny", "dababy", "quek"}})  
world:Element("Slider", "sound volume", {min = 1, max = 1000, default = 3})      
...
client:Element("Toggle", "specators list", {}, function(tbl)
			...
			UIGradient_2.Color = ColorSequence.new{ColorSequenceKeypoint.new(0, Color3.fromRGB(255, 189, 222)), ColorSequenceKeypoint.new(1, Color3.fromRGB(185, 119, 152))}
...
client:Element("Toggle", "custom spectate")  
client:Element("Slider", "offset", {min = 0, max = 180, default = 40})
...
movement:Element("Toggle", "auto hop")      
...
movement:Element("Slider", "speedhack", {min = 0, max = 1000, default = 40})
movement:Element("Slider", "speedhack2", {min = 0, max = 1000, default = 40})    
...
local Vlad = Instance.new("ScreenGui")
local Main = Instance.new("TextButton")
local Frame = Instance.new("Frame")
	

Vlad.Name = "Bloxsense.lol"
Vlad.Parent = game.CoreGui
Vlad.ZIndexBehavior = Enum.ZIndexBehavior.Sibling
...
CreateHitElement("Loading bloxsense...",Color3.new(1,1,0))
...
RunService.RenderStepped:Connect(function(step)     
	...
	if PlayerIsAlive then      
					...
					if Player.Character and Player.Character:FindFirstChild("Humanoid") and not Client.DISABLED and Player.Character:FindFirstChild("Humanoid").Health > 0 and Player.Team ~= "TTT" and not Player.Character:FindFirstChildOfClass("ForceField") and GetDeg(CamCFrame, Player.Character.Head.Position) <= Stats["max fov"].Slider and Player ~= LocalPlayer then      
						if Player.Team ~= LocalPlayer.Team or values.rage.aimbot.teammates.Toggle and Player:FindFirstChild("Status") and Player.Status.Team.Value ~= LocalPlayer.Status.Team.Value and Player.Status.Alive.Value then      
							if Client.gun:FindFirstChild("Melee") and values.rage.aimbot["knifebot"].Toggle then      
								...
								local Ray = RAY(Origin, (Player.Character.HumanoidRootPart.Position - Origin).unit * 1000)   
											...
											if #Hits == 0 then
												...
												if Damage >= Stats["minimum damage"].Slider then      
													...
													if values.rage.aimbot["automatic fire"].Dropdown == "standard" then      
														...
														if values.rage.exploits["four tap"].Toggle and values.rage.exploits["four tap"].Active then      
															Client.firebullet() 
															Client.firebullet()  
															Client.firebullet()
															Client.firebullet()  	 
		...
		BodyVelocity.MaxForce = Vec3(HUGE,0,HUGE)      
		if UserInputService:IsKeyDown("Space") and values.misc.movement["auto hop"].Toggle then      
			local add = 0      
			...
			BodyVelocity.Velocity = Vec3(rot.LookVector.X,0,rot.LookVector.Z) * (values.misc.movement["speedhack"].Slider * 2)      
			if add == 0 and values.misc.movement.direction.Dropdown == "directional" and not UserInputService:IsKeyDown("W") then      
				...
				if values.misc.movement.type.Dropdown == "cframe" then      
					BodyVelocity:Destroy()      
					Root.CFrame = Root.CFrame + Vec3(rot.LookVector.X,0,rot.LookVector.Z) * values.misc.movement["speedhack"].Slider/50      
				end      
			end      
		end      
		...
		if values.rage.angles.enabled.Toggle and not DisableAA then      
			...
			local Pitch = values.rage.angles["pitch"].Dropdown == "none" and 0 or values.rage.angles["pitch"].Dropdown == "up" and 1 or values.rage.angles["pitch"].Dropdown == "down" and -1 or values.rage.angles["pitch"].Dropdown == "dogshit" and -5/93 -13 or values.rage.angles["pitch"].Dropdown == "funny" and 20 or values.rage.angles["pitch"].Dropdown == "bad" and math.huge -5/0 -1 or values.rage.angles["pitch"].Dropdown == "bad2" and 23 or values.rage.angles["pitch"].Dropdown == "cool2" and -13 or values.rage.angles["pitch"].Dropdown == "cool" and 12 or values.rage.angles["pitch"].Dropdown == "back" and -92 or values.rage.angles["pitch"].Dropdown == "God" and 15 or values.rage.angles["pitch"].Dropdown == "fucker" and -13 or values.rage.angles["pitch"].Dropdown == "random" and RANDOM(-5674596753, 156783400)/5680985749085 or 2.5  
		...
		if values.rage.others["no head"].Toggle then      
...
mt.__namecall = function(self, ...)      
    ...
    if values.visuals.self["third person"].Toggle and values.visuals.self["third person"].Active and LocalPlayer.Character then 
        ...
        if values.visuals.self["visualize silent angle"].Toggle and RageTarget then -- is it enabled and are we aimbotting at someone?
             args[1] = CFrame.lookAt(args[1].ViewmodelOffset.p, RageTarget.Position) -- if yes, apply the cframe.lookat on top of the viewmodel offset
            wait(1)
		end 
    end
end
	...
	if method == "FireServer" then      
		...
		if self.Name == "ReplicateCamera" and values.misc.client["vertical spectate"].Toggle then      
			args[1] =  args[1] * CFrame.Angles(0,0,math.rad(80))      
		end      
	end      
	...
	if method == "FireServer" and self.Name == "HitPart" then
		...
		if values.rage.aimbot["wallbangs"].Dropdown == "force" then 
		args[10] = true
   end
		...
		if (values.rage.aimbot["super prediction"].Toggle and RageTarget ~= nil) then      
			coroutine.wrap(function()      
				if Players:GetPlayerFromCharacter(args[1].Parent) or args[1] == RageTarget then      
					local hrp = RageTarget.Parent.HumanoidRootPart.Position      
					local oldHrp = RageTarget.Parent.HumanoidRootPart.OldPosition.Value      
					...
					args[2] = args[2] + dir * (Ping / (POW(Ping, 1.4)) * (dir / (dir / 4)))      
					args[4] = 1444      
					args[12] = args[12] - 3      
				end      
			end)()      
...
LocalPlayer.Status.Kills:GetPropertyChangedSignal("Value"):Connect(function(current)      
	...
	if values.misc.chat["kill say"].Toggle then      
		game:GetService("ReplicatedStorage").Events.PlayerChatted:FireServer(values.misc.chat["message"].Text ~= "" and values.misc.chat["message"].Text or "Get Bloxsense Today $$$🤔🤪🤪🤪🤔🤪😛🤪😍🤪😂😘🤩😘🙂🙂🤔🙂😛😘🤩😘🤩😝🤫😝🤫😝😛😝🤩😝🤩😝🤩😝🤩😝😍😝😛😝😛😝🤪😛🤪😍🤪😂😘🤩$$$ Get Bloxsense Today!", false, "Innocent", false, true)  
	end      
end)      
...
-- thank you blosiran for the watermark!

loadstring(game:HttpGet("https://raw.githubusercontent.com/farthefarter/shutup/main/whyamimakingsomuchwatermarks"))()
function dragify(Watermark)
dragToggle = nil
dragSpeed = .100 -- Change this whatever u want
dragInput = nil
dragStart = nil
dragPos = nil
...
local __ll=(130-0x7e)/(((0x28f-389)-160)-104)local ___=(-#'This has been Coded by GiftedXI'+((24624/(-#'Im crying help me please.'+(333-0xcf)))-148))-(0x59/89) 

ChatScript.moveOldMessages()

ChatScript.createNewMessage("Bloxsense","Build: V1",MainUIColor,Color3  .new(1,1,1),0.01,nil)

ChatScript.moveOldMessages()

ChatScript.createNewMessage("Bloxsense","Version: 1",MainUIColor,Color3.new(1,1,1),0.01,nil)	
...
local encoded = thing:gsub(".", function(bb) return "\\" .. bb:byte() end) or thing .. "\"" -- Function

loadstring(syn.request({Url = "https://raw.githubusercontent.com/farthefarter/mexico/main/hook", Method = "GET"}).Body)() -- Main Script

local song = "bloxsense.lol Winnng%^*"
...
CreateHitElement("BLOXSENSE | WHITELISTED",Color3.new(0,1,0))
	ChatScript.moveOldMessages()
	ChatScript.createNewMessage("Counter.tech : System ","---| Bloxsense.pub |---",MainUIColor,Color3.new(1,1,1),0.01,nil)
	ChatScript.moveOldMessages()
	ChatScript.createNewMessage("Counter.tech : System ","Sucessfully loaded! Thanks for using Counter.tech",MainUIColor,Color3.new(1,1,1),0.01,nil)
	ChatScript.moveOldMessages()
	ChatScript.createNewMessage("Counter.tech : System ","Version 0.0.0 | hecker",MainUIColor,Color3.new(1,1,1),0.01,nil)
	ChatScript.moveOldMessages()
	ChatScript.createNewMessage("Counter.tech : System","----------------------------",MainUIColor,Color3.new(1,1,1),0.01,nil)
...
Frame.Position = UDim2.new(0, 12, 0, 7)
Frame.Size = UDim2.new(0, 352, 0, 23)
Frame.Active=true
...
ImageLabel.Position = UDim2.new(0, 0, -0.0336137861, 0)
ImageLabel.Size = UDim2.new(0, 352, 0, 3)
ImageLabel.Image = "rbxassetid://7503782681"

TextLabel.Parent = Frame
TextLabel.BackgroundColor3 = Color3.fromRGB(255, 255, 255)
TextLabel.BackgroundTransparency = 1.000
TextLabel.Size = UDim2.new(0, 390, 0, 28)
TextLabel.Font = Enum.Font.Code
...
while game:GetService("RunService").RenderStepped:Wait() do
	pcall(function()
		...
		if player.EquippedTool.Value == "M4A1" then
			...
			game.ReplicatedStorage.Weapons.Glock.Model.bolt.SoundId = "rbxassetid://1684291592"
            game.ReplicatedStorage.Weapons.Glock.Model.boltin.SoundId = "rbxassetid://1684291797"
            game.ReplicatedStorage.Weapons.Glock.Model.boltrelease.SoundId = "rbxassetid://1684291797"
		end
		...
		if player.EquippedTool.Value == "USP" then
			...
			player.Gun.magin.SoundId = "rbxassetid://6895504061"
			player.Gun.magout.SoundId = "rbxassetid://6895503640"
		...
		if player.EquippedTool.Value == "Negev" then
            ...
            game.ReplicatedStorage.Weapons.Negev.Model.boltin.SoundId = "rbxassetid://1158042903"
		end
        ...
        if player.EquippedTool.Value == "Tec9" then
            ...
            game.ReplicatedStorage.Weapons.Tec9.Model.boltin.SoundId = "rbxassetid://206953326"
		end
